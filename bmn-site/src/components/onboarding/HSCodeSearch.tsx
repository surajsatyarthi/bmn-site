                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-text-primary">
                      {item.code.length === 2 ? `Chapter ${item.code}` : item.code}
                    </span>
                    {item.code.length === 2 ? (
                      <span className="text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-gray-100 text-gray-600">Chapter</span>
                    ) : (
                      <span className="text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-blue-100 text-bmn-blue">Detail</span>
                    )}
                  </div>
                  <div className="text-text-secondary text-xs line-clamp-2 mt-0.5">{item.description}</div>
                </div>
              </button>
            );
          })}
